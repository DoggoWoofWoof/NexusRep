...
export async function GET(): Promise<NextResponse> {
  ...
  // Persisted video-off voice (Agent gallery). The doctor view's off-video TTS speaks in it; with
  // voiceWholeConvo it's used for the whole conversation (video on too).
  const voiceId = snap?.appearance?.voiceId ?? undefined;
  const voiceWholeConvo = snap?.appearance?.voiceWholeConvo ?? undefined;
