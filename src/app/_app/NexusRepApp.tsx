...
export function NexusRepApp() {
  ...
  const [sessionHcpId, setSessionHcpId] = useState("sharma");
  ...
  return (
    <div style={{ height: "100vh", overflow: "hidden", display: "flex", background: "var(--dn-bg)", color: "var(--dn-fg)", fontFamily: "var(--dn-font-sans)" }}>
      {/* LEFT NAV RAIL */}
      <aside style={{ width: 236, flexShrink: 0, background: "var(--dn-brand-dark)", color: "#fff", display: "flex", flexDirection: "column", borderRight: "1px solid rgba(255,255,255,.07)" }}>
        <div style={{ padding: "17px 18px 15px", borderBottom: "1px solid rgba(255,255,255,.08)", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <nav style={{ flex: 1, overflowY: "auto", padding: 10, display: "flex", flexDirection: "column", gap: 1 }}>
          {NAV_PLAN.map((item) => <NavRow key={item.id} item={item} active={nav === item.id} onClick={() => setNav(item.id)} />)}
          <div style={{ height: 1, background: "rgba(255,255,255,.08)", margin: "12px 8px 4px" }} />
          <div style={{ font: "600 10px/1 var(--dn-font-sans)", letterSpacing: ".12em", color: "rgba(255,255,255,.38)", padding: "8px 12px 7px" }}>ACTIVITY</div>
          {NAV_GOVERN.map((item) => <NavRow key={item.id} item={item} active={nav === item.id} onClick={() => setNav(item.id)} />)}
        </nav>
        <div style={{ padding: "8px 10px", borderTop: "1px solid rgba(255,255,255,.08)" }}>
          <div onClick={() => setNav("admin")} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 13px", borderRadius: 9, cursor: "pointer", font: "500 13px/1.2 var(--dn-font-sans)", color: nav === "admin" ? "#fff" : "rgba(255,255,255,.6)", background: nav === "admin" ? "rgba(96,165,250,.18)" : "transparent" }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#64748b" }} />Platform Admin
            <span style={{ font: "500 9.5px/1 var(--dn-font-sans)", color: "rgba(255,255,255,.4)", marginLeft: "auto" }}>INTERNAL</span>
          </div>
        </div>
        <div style={{ padding: "11px 14px", borderTop: "1px solid rgba(255,255,255,.08)", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <header style={{ height: 60, flexShrink: 0, background: "#fff", borderBottom: "1px solid var(--dn-border)", display: "flex", alignItems: "center", padding: "0 22px", gap: 16 }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ position: "relative" }}>
              <span onClick={() => setAttnOpen((v) => !v)} style={{ display: "flex", alignItems: "center", gap: 6, font: "600 11px/1 var(--dn-font-sans)", color: "#92400e", background: "var(--dn-accent-yellow-bg)", padding: "6px 11px", borderRadius: 20, cursor: "pointer" }}><span style={{ width: 7, height: 7, borderRadius: "50%", background: "var(--dn-warning)" }} />Needs attention: 3</span>
              {attnOpen && (
                <div style={{ position: "absolute", top: 34, right: 0, width: 250, background: "#fff", border: "1px solid var(--dn-border)", borderRadius: 11, boxShadow: "var(--dn-shadow-popover)", padding: 8, zIndex: 30 }}>
                  <AttnItem n="1" label="MLR item pending review" onClick={() => { setAttnOpen(false); setNav("studio"); }} />
                  <AttnItem n="2" label="CRM export issues" onClick={() => { setAttnOpen(false); setNav("crm"); }} />
                </div>
              )}
            </span>
...
function NavRow({ item, active, onClick }: { item: { id: Screen; label: string; badge?: string }; active: boolean; onClick: () => void }) {
  return (
    <div onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 13px", borderRadius: 9, font: `${active ? 600 : 500} 13px/1.2 var(--dn-font-sans)`, cursor: "pointer", position: "relative", color: active ? "#fff" : "rgba(255,255,255,.72)", background: active ? "rgba(96,165,250,.18)" : "transparent" }}>
      <span style={{ position: "absolute", left: 0, top: 8, bottom: 8, width: 3, borderRadius: 2, background: active ? "#60a5fa" : "transparent" }} />
      {item.label}
      {item.badge && <span style={{ marginLeft: "auto", font: "700 9px/1 var(--dn-font-sans)", letterSpacing: ".04em", padding: "3px 6px", borderRadius: 5, background: "rgba(96,165,250,.22)", color: "#bfdbfe" }}>{item.badge}</span>}
    </div>
  );
}
...
function useCommandKpis(): CommandKpi[] {
...
function OverviewScreen({ app }: { app: AppState }) {
  const commandKpis = useCommandKpis();
  const brand = useBrand();
  return (
    <div style={{ padding: "24px 30px 40px", maxWidth: 1340 }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 18 }}>
        <div>
          <div style={{ font: "600 11px/1.2 var(--dn-font-sans)", letterSpacing: ".08em", textTransform: "uppercase", color: "var(--dn-brand-light)", marginBottom: 6 }}>Command Center</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.62fr 1fr", gap: 16 }}>
        <div style={{ background: "#fff", border: "1px solid var(--dn-border)", borderRadius: 13, padding: "18px 20px", boxShadow: "var(--dn-shadow-card)" }}>
          <div style={{ font: "600 13px/1 var(--dn-font-sans)", marginBottom: 4 }}>What HCPs are asking</div>
          <div style={{ font: "400 11.5px/1.3 var(--dn-font-sans)", color: "var(--dn-fg-subtle)", marginBottom: 16 }}>Top approved topics across completed sessions</div>
        ...
        <div style={{ background: "#fff", border: "1px solid var(--dn-border)", borderRadius: 13, padding: "18px 20px", boxShadow: "var(--dn-shadow-card)" }}>
          ...
          {HCPS.slice(0, 3).map((h) => (
            <div key={h.id} onClick={() => app.setNav("sessions")} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid var(--dn-surface-2)", cursor: "pointer" }}>
              <span style={{ width: 28, height: 28, borderRadius: "50%", background: "var(--dn-surface-2)", display: "flex", alignItems: "center", justifyContent: "center", font: "600 11px/1 var(--dn-font-sans)", color: "var(--dn-brand-base)" }}>{h.name.split(" ").slice(-1)[0]?.[0]}</span>
              <div style={{ flex: 1 }}>
                <div style={{ font: "600 12px/1.2 var(--dn-font-sans)" }}>{h.name}</div>
                <div style={{ font: "400 10.5px/1.2 var(--dn-font-sans)", color: "var(--dn-fg-subtle)", marginTop: 2 }}>{h.topic}</div>
              </div>
              <span style={{ font: "600 11px/1 var(--dn-font-sans)", color: "var(--dn-brand-light)" }}>Coach →</span>
            </div>
          ))}
        </div>
      </div>
