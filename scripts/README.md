...
| `scripts/verify-client-recording.mjs` | **Verifies the CLIENT-side recording end-to-end**: opens `/hcp`, records via the app's own `recordSession`, clicks End video, and confirms a `/recordings/capture-*.webm` attached to the session. This is our recording path (Tavus's own recording is off — see below). Needs Tavus credits (~1 short call). |
